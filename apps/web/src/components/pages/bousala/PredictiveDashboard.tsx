...
import { buildForecastHistoryData } from '../../../lib/bousalaDemoData';
...
const PredictiveDashboard: React.FC<PredictiveDashboardProps> = ({ goals, onGoToDashboard }) => {
    const { t } = useLocalization(['bousala']);
    const forecastHistory = useMemo(() => buildForecastHistoryData(t), [t]);
    ...
    const hasPredictions = useMemo(
        () => goals.some(g => g.prediction !== undefined || (g.kpis && g.kpis.some(k => k.prediction !== undefined))),
        [goals],
    );
    ...
    if (goals.length === 0) {
        return (
            <BousalaSectionEmpty
                title={t('bousala.predictive.noGoalsTitle')}
                description={t('bousala.predictive.noGoalsDescription')}
                actionLabel={onGoToDashboard ? t('bousala.predictive.goToDashboard') : undefined}
    ...
    if (!hasPredictions) {
        return (
            <BousalaSectionEmpty
                title={t('bousala.predictive.noPredictionsTitle')}
                description={t('bousala.predictive.noPredictionsDescription')}
                actionLabel={onGoToDashboard ? t('bousala.predictive.goToDashboard') : undefined}
                onAction={onGoToDashboard}
            />
        );
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-card dark:bg-dark-card/50 p-6 rounded-2xl shadow-soft border dark:border-slate-700/50">
                    <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                        <AlertCircle className="text-red-500" /> {t('bousala.predictive.topAtRiskGoals')}
                    </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-2 bg-card dark:bg-dark-card/50 p-6 rounded-2xl shadow-soft border dark:border-slate-700/50">
                    <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                        <Bot className="text-primary" /> {t('bousala.predictive.aiRecommendationsSummary')}
                    </h3>
